...
|---|---|---|---|
| A1 | Human listener | Trained forensic phonetician | 7-layer transform alters all perceptual vocal cues |
| A2 | ASV system | ECAPA-TDNN or x-vector embedder | Formant warp + breathiness destroy embedding geometry; COWL removes residual cues |
| A3 | Parameter inverter | Knows all transformation parameters; attempts to undo them | COWL is applied after vocoder — irreversible even with full parameter knowledge |
| A4 | Key guesser | Brute-force or dictionary attack on passphrase | HKDF-SHA256 provides 2¹²⁸ preimage resistance |
...
- **Second-preimage resistance:** finding a different `key` that produces the same parameters requires colliding HMAC-SHA256 (2¹²⁸ security).
- **Domain separation:** each parameter uses a unique label, ensuring statistical independence across all eight parameters even for short keys.
...
Let `T(S, params)` be the vocoder transform output.
Let `C = COWL(T(S, params), params_cowl)` be the final output.

Recovering `S` from `C` requires all three of the following simultaneously:

**Step 1 — Invert NLSQ:** Given the μ-law-quantised magnitude spectrum,
recover the original magnitude. This is a many-to-one mapping (many source
values map to the same quantised bin). The problem is underdetermined without
additional constraints.

**Step 2 — Undo Phase Randomisation:** Recover the phase perturbation
`Δφ ~ N(0, 0.15π × intensity)` applied to each STFT bin. The perturbation
seed is derived from `HMAC-SHA256(seed_bytes, "chimera:cowl:phase")`.
Without the key, the seed is unknown, reducing the problem to blind phase
retrieval — NP-hard in general.

**Step 3 — Remove SSNI noise:** The injected noise is spectrally shaped to
the ISO 226 masking curve and scaled by the key-derived noise PRK. Removing
it requires knowing the noise PRK, which is derived from the key. Without it,
single-channel noise separation is an ill-posed problem.

No polynomial-time algorithm accomplishing all three steps simultaneously is
