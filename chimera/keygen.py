...
def derive_params(
    ...
    *,
    ...
    # Ranges chosen to span the full perceptual space of human vocal
    # production without exceeding naturalness bounds.
    pitch = _expand_float(prk, "pitch", -10.0, 10.0)
    formant = _expand_float(prk, "formant", 0.78, 1.22)
    tilt = _expand_float(prk, "tilt", -4.0, 4.0)
    breathiness = _expand_float(prk, "breathiness", 0.0, 0.45)
    jitter = _expand_float(prk, "jitter", 0.0, 0.018)
    vib_rate = _expand_float(prk, "vibrato_rate", 0.0, 7.0)
    vib_depth = _expand_float(prk, "vibrato_depth", 0.0, 0.40)
    subharmonic = _expand_float(prk, "subharmonic", 0.0, 0.15)
    ...
    raw = MaskParams(
        ...
        subharmonic_mix=subharmonic,
        intensity=intensity,
        seed=seed,
        speaker_label=speaker_label,
    )

    # Apply intensity scaling after derivation so that I=0 is always identity
    return raw.scaled(intensity)
